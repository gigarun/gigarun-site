...
key = keys[0]
UA = {"User-Agent": "Mozilla/5.0 (compatible; gigarun-indexnow/1.0)"}  # Cloudflare bloque le UA Python par defaut
urls = sys.argv[1:] or re.findall(r"<loc>([^<]+)</loc>", urllib.request.urlopen(urllib.request.Request(f"https://{HOST}/sitemap-0.xml", headers=UA)).read().decode())
body = json.dumps({"host": HOST, "key": key, "keyLocation": f"https://{HOST}/{key}.txt", "urlList": urls}).encode()
req = urllib.request.Request("https://api.indexnow.org/indexnow", body, {**UA, "Content-Type": "application/json; charset=utf-8"})
